        ...
        try {//try 不能删除，不然会出现兼容性问题
            if (content instanceof CoordinatorLayout) {
                ...
                for (int i = layout.getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (view instanceof AppBarLayout) {
                    ...
                    } else if (view instanceof io.github.iamyours.flingappbarlayout.AppBarLayout) {
                        ((io.github.iamyours.flingappbarlayout.AppBarLayout) view).addOnOffsetChangedListener(new io.github.iamyours.flingappbarlayout.AppBarLayout.OnOffsetChangedListener() {
                            @Override public void onOffsetChanged(io.github.iamyours.flingappbarlayout.AppBarLayout appBarLayout, int verticalOffset) {
                                listener.onCoordinatorUpdate(
                                        verticalOffset >= 0,
                                        (appBarLayout.getTotalScrollRange() + verticalOffset) <= 0);
                            }
